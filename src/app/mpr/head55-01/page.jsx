...
const ACTIVITIES = [
  { code:'55-01', name:'विस्तृत परियोजना रिपोर्ट पर व्यय', en:'DPR Preparation', hasPhysical:false, hasSize:false },
  ...
  { code:'55-01(06)', name:'अस्थाई चेक डेम (पिरुल आदि)', en:'Temporary Check Dam', hasPhysical:true, hasSize:false, unit:'No.' },
...
export default function MPRPraroop1A() {
  ...
  const [totalSpringsUnderSchemes, setTotalSpringsUnderSchemes] = useState('');
  const [springsCurrentlyBeingTreated, setSpringsCurrentlyBeingTreated] = useState('');
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        if (parsed.totalSpringsUnderSchemes) setTotalSpringsUnderSchemes(parsed.totalSpringsUnderSchemes);
        if (parsed.springsCurrentlyBeingTreated) setSpringsCurrentlyBeingTreated(parsed.springsCurrentlyBeingTreated);
  ...
  const fetchBaseline = async () => {
      try {
          const res = await axiosInstance.get(`/mpr/praroop1a/baseline?financialYear=${financialYear}`);
  ...
  const handleCopyLastMonth = async () => {
      try {
          const res = await axiosInstance.get(`/mpr/praroop1a/previous-month?financialYear=${financialYear}&month=${month}`);
          if(res.data && res.data.data) {
  ...
  const formatPayload = (isDraft = false) => {
    return {
      ...
      totalSpringsUnderSchemes: parseInt(totalSpringsUnderSchemes) || 0,
      springsCurrentlyBeingTreated: parseInt(springsCurrentlyBeingTreated) || 0,
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      await axiosInstance.post('/mpr/praroop1a/draft', payload);
  ...
  const handleSubmit = async () => {
    if (!totalApprovedSchemes || !totalSpringsUnderSchemes || !springsCurrentlyBeingTreated) {
    ...
    try {
      ...
      const res = await axiosInstance.post('/mpr/praroop1a/submit', payload);
      localStorage.removeItem(`sarra_mpr_praroop1a_${user?._id || 'guest'}_${financialYear}_${month}`);
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6">
        <div className="max-w-md w-full bg-white rounded-3xl shadow-xl p-8 text-center border border-slate-100">
          ...
          <h2 className="text-2xl font-bold text-slate-800 mb-2">MPR Praroop-1(A) Submitted!</h2>
          <p className="text-slate-500 mb-6">Your monthly progress report has been successfully submitted.</p>
    ...
    <div className="min-h-screen bg-slate-50 pb-20">
      ...
      <div className="bg-[#0a3d62] px-6 py-6 z-50 sticky top-0 shadow-md">
        <div className="max-w-[98%] mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-xl md:text-2xl font-bold text-white tracking-tight">प्रारूप-1(A) | HEAD 55-01 | MPR</h1>
            <p className="text-blue-200 text-sm mt-1">SARRA 55-पूंजीगत परिसम्पत्तियों — वित्तीय एवं भौतिक प्रगति विवरण</p>
          </div>
      ...
      <div className="max-w-[98%] mx-auto mt-6">
          ...
          <div className="bg-[#FFF9C4] p-4 rounded-xl shadow-sm border border-amber-200 mb-6 flex flex-wrap gap-4 items-center">
             ...
             <div className="flex-1 min-w-[200px]">
                 <label className="block text-xs font-bold text-amber-900 mb-1">2. आच्छादित कुल स्प्रिंग की संख्या</label>
                 <input type="number" min="0" value={totalSpringsUnderSchemes} onChange={e => setTotalSpringsUnderSchemes(e.target.value.replace(/-/g, ''))} className="w-full px-3 py-2 rounded border border-amber-300 focus:ring-2 focus:ring-[#e67e22] outline-none" />
             </div>
             <div className="flex-1 min-w-[200px]">
                 <label className="block text-xs font-bold text-amber-900 mb-1">3. उपचार किये जा रहे स्प्रिंग की संख्या</label>
                 <input type="number" min="0" value={springsCurrentlyBeingTreated} onChange={e => setSpringsCurrentlyBeingTreated(e.target.value)} className="w-full px-3 py-2 rounded border border-amber-300 focus:ring-2 focus:ring-[#e67e22] outline-none" />
             </div>
          </div>
